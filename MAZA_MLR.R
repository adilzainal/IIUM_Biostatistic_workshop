...
#Previously we have seen that there is a linear relationship between systolic blood pressure and HbA1C  
attach(data)
...
# So if a person with HbA1c value of 10, the systolic blood pressure is = 116.5 + 1.05(10) = 127mmHg
...
contrasts(exercise)
data <- within(data, exercise <- relevel(exercise, ref = "High"))
contrasts(exercise)
# Now i would like undo that.
data <- within(data, exercise <- relevel(exercise, ref = "Low"))
