...
sbpc <- cut(sbp , breaks = c(-Inf,139.9,Inf), labels = c("1","2"))
sbpc <-as.numeric(sbpc)
dbpc <- cut(dbp , breaks = c(-Inf,89.9,Inf), labels = c("1","2"))
dbpc <-as.numeric(dbpc)
bpc <- dbpc + sbpc
hpt <- cut(bpc, breaks = c(-Inf,2,Inf), labels = c("Normotensive","Hypertension"))
hpt <- as.factor(hpt)
summary(hpt)
...
# Having moderate exercise compare to low exercise, change the log odds of hypertension by -2.1762
...
# After controlled for HbA1c, those who practice high exercise intensity have 0.07 lower odds compared to those in low exercise to get hypertension.
